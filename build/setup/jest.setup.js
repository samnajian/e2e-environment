...
var _jestObject = require("../../config/jest-object.config");
...
(0, _utils.addConsoleSuppression)('Failed to load resource: net::ERR_PROXY_CONNECTION_FAILED');
(0, _utils.addConsoleSuppression)('the server responded with a status of 404');
/**
...
async function setupBrowser() {
  await (0, _e2eTestUtils.clearLocalStorage)();
  await (0, _e2eTestUtils.setBrowserViewport)({
    width: 1280,
    height: 800
  });
}
...
function observeConsoleLogging() {
  page.on('console', message => {
    ...
    if (text.includes('net::ERR_INTERNET_DISCONNECTED') && (0, _e2eTestUtils.isOfflineMode)()) {
      return;
    ...
    if ((0, _utils.consoleShouldSuppress)(text)) {
      return;
    ...
    text = (0, _lodash.get)(message.args(), [0, '_remoteObject', 'description'], text); // Disable reason: We intentionally bubble up the console message
...
beforeAll(async () => {
  ...
  (0, _e2eTestUtils.enablePageDialogAccept)();
  observeConsoleLogging();
  (0, _jestObject.setupJestRetries)();
});
afterEach(async () => {
  await setupBrowser();
});
